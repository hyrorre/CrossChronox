...
const int* GetJudgeTable(lane_t lane, bool lnend_flag, Mode mode) {
    if (mode <= BEAT_14K) { // if bms
        ...
        // if normal note
            ...
            return (lnend_flag ? bms_lnend_judge_table : bms_judge_table);
    }
    // if pms
    else
        return (lnend_flag ? pms_lnend_judge_table : pms_judge_table);
}
    ...
    if (!lane_timeline.empty()) {
        ...
        for (Note* note : lane_timeline) {
            ...
            // LNが押されている時
                ...
                if (0 < key_state.now) {
                    // lnendを過ぎたら
                    if (note->lnend_ms < play_ms)
                        ln_finish_pushing_flag = true;
                }
                // キーが離された時
                else {
                    // lnendに近いところで離したらBADにはならない
                    if (note->lnend_ms < play_ms + GetJudgeTable(note->lane, true, mode)[GOOD])
                        ln_finish_pushing_flag = true;
                    // 離すのが早すぎるとBAD
                    else {
