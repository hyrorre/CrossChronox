...
F1メニューのプロファイル設定にある「キー設定」でも、共通操作と4K / 5K / 6K / 7K /
8K / 9K / 10K / 14Kの割り当てを変更できます。対象と入力スロットを選び、現在値のボタンを
押してからキーまたはコントローラを入力します。Escapeで待受をキャンセルし、Delete /
Backspaceまたは「解除」で割り当てを削除します。Decide / Play中は変更できません。
...
| 6 | 現在のフォルダ階層を維持したまま `設定 → キー設定` を開く |
...
F3 / F5 / F10 / F11 / 1 / 2 / 3 / 4（Numpad4を含む）/ 6 / 7（Numpad7を含む）/
8（Numpad8を含む）/ 9（Numpad9を含む）は既定割り当てです。これらも
`設定 → キー設定 → 共通` から変更できます。

対応スキンの `KEY CONFIG` ボタンは、現在のフォルダ階層を維持したまま
`設定 → キー設定` を開きます。キー設定の一覧から戻ると、ボタンを押す直前のフォルダと
カーソル位置へ戻ります。
