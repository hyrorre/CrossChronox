...
左サイドメニューから一般／プロファイル／音声／映像／入力／プレイ／選曲／スキン／
ライブラリ／連携・録画／データ移行／更新・診断を選びます。幅が狭い場合は左メニューが
カテゴリ選択のプルダウンになります。項目数が多いページは上部のプルダウンで内容を
切り替えます。設定群は折りたたまずに表示し、ページごとのスクロール位置を保持します。

「すべての変更を保存」は他のページも含めて本体設定・プロファイル設定を保存します。
未保存の変更があるカテゴリには印が付き、保存の成功・失敗は下部に表示されます。
...
選び、その対象だけを編集します。「表示内容」でオプション／素材／位置調整を切り替え、
検索欄で項目名を絞り込めます。検索語と表示分類は対象ごとに保持されます。
「全スキンを保存済みの設定に戻す」は全対象を保存済みの値へ戻す操作です。
...
ゲーム内設定またはF1メニューの設定 → 入力では、`gamepad1` (1P) / `gamepad2` (2P)
...
F1メニューの設定 → 入力 → キー設定でも、共通操作と4K / 5K / 6K / 7K /
